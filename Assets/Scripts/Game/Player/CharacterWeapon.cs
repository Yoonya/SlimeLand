...
public class CharacterWeapon : MonoBehaviour
{
    ...
    //일반 총알 0,1,2는 오브젝트풀에서 type로 빼낼 수 있지만, 장판과 검 무기는 따로 오브젝트를 넣다뺏다 해야한다.
    //0 = element, 1 = fire, 2 = elements, 3 = fires, 4 = swordL, 5 = swordR, 6 = elementP, 7 = fireP
    public GameObject[] weapons;

    public Transform characterLT; //캐릭터의 위치;
    public Transform characterDirect; //캐릭터의 위치;
    ...
    public float exTime = 0f;
    private Collider2D[] exRangeCol; //폭발 공격의 범위
    ...
    void Update()
    {
                ...
                if (sideWeaponTypeR != -1) //오른쪽 무기가 있다면
                {
                    ...
                    if ((sideWeaponTypeR == 4 || sideWeaponTypeR == 5) && weaponTypeR >= -1 && weaponTypeR <= 2) //발사형탄환일때 장판을 생성
                    {
                        ...
                        if (sideFloorWeapon2.childCount != 0)
                        {
                            ...
                            CreateSideFloorBullet(sideWeaponTypeR - 4, 1); //장판 오른쪽
                    }   
        ...
        if (characterStatus.equipCombo == 4) //화염 슬라임 전용
        ...
        if (exTime >= 3.0f)
        {
            if (!GameManager.instance.isEnd)
            {
                if (!GameManager.instance.isPause)
                {
                    ...
                    exRangeCol = Physics2D.OverlapCircleAll(gameObject.transform.position, 6.0f); //범위 안에 있으면 공격
                    for (int i = 0; i < exRangeCol.Length; i++)
                    {
                        if (exRangeCol[i].CompareTag("Enemy")) //적이라면
                        {
                            if (critRand <= characterStatus.crit)
                            {
                                exRangeCol[i].GetComponent<Enemy>().EnemyHitted(characterStatus.atk * 2, characterStatus.equipCombo);
                                DamageText(exRangeCol[i].gameObject, characterStatus.atk * 2);
                            }
                            else
                            {
                                exRangeCol[i].GetComponent<Enemy>().EnemyHitted(characterStatus.atk, characterStatus.equipCombo);
                                DamageText(exRangeCol[i].gameObject, characterStatus.atk);
                            }
                        }
                    }
                    exEffect.transform.position = gameObject.transform.position;
        ...
        if (slashTime >= 5.0f)
        {
            if (!GameManager.instance.isEnd)
            {
                if (!GameManager.instance.isPause)
                {
                    slashRangeCol = Physics2D.OverlapCircleAll(gameObject.transform.position, 3.0f); //범위 안에 있으면 공격  
                    ...
                    {
                        if (slashRangeCol[i].CompareTag("Enemy")) //적이라면
                        {
    ...
    IEnumerator SlashAttack(List<Collider2D> slashAttack)
    {
        ...
        for (int i = 0; i < slashAttack.Count; i++)
        {
            if (slashAttack[i].gameObject.activeSelf == true) //살아있는 적만
            {
                if (critRand <= characterStatus.crit)
                {
                    slashAttack[i].GetComponent<Enemy>().EnemyHitted(characterStatus.atk * 5 * 2, characterStatus.equipCombo);
                    DamageText(slashAttack[i].gameObject, characterStatus.atk * 5 * 2);
                }
                else
                {
                    slashAttack[i].GetComponent<Enemy>().EnemyHitted(characterStatus.atk * 5, characterStatus.equipCombo);
                    DamageText(slashAttack[i].gameObject, characterStatus.atk * 5);
                }
            }
        }
    ...
    private void CreateSideBullet(int weaponType, int dir)
    {
        if (characterStatus.hiddenCombo != 13) //히든런처가 아니라면
        {
            ...
            tempBullet.GetComponent<SpriteRenderer>().color = new Color(1f, 1f, 1f, 1f);
    ...
    private void CreateBasicBullet()
    {
        if (characterStatus.hiddenCombo != 13) //히든런처가 아니라면
        {
            ...
            tempBullet.transform.localEulerAngles = characterDirect.localEulerAngles; //캐릭터가 바라보는 방향으로 발사 조정
            tempBullet.transform.position = characterLT.transform.position; //시각적으로 캐릭터보다 앞에서 발사
            tempBullet.GetComponent<Bullet>().speed = characterStatus.spd * 2f;
            tempBullet.GetComponent<Bullet>().atk = characterStatus.atk;
            tempBullet.GetComponent<Bullet>().crit = characterStatus.crit;
            tempBullet.GetComponent<Bullet>().istriggered = false;
        ...
        else //히든런처라면
        {
            ...
            tempBullet.transform.localEulerAngles = characterDirect.localEulerAngles; //캐릭터가 바라보는 방향으로 발사 조정
            tempBullet.transform.position = characterLT.transform.position + transform.up; //시각적으로 캐릭터보다 앞에서 발사
            tempBullet.GetComponent<Bullet>().speed = characterStatus.spd * 2f;
            tempBullet.GetComponent<Bullet>().atk = characterStatus.atk;
            tempBullet.GetComponent<Bullet>().crit = characterStatus.crit;
            tempBullet.GetComponent<Bullet>().weaponCombo = characterStatus.weaponCombo;
    ...
    public void CreateSword(int type)
    {
        if (type == 2) //콤보효과라면
        {
            GameObject sword1 = Instantiate(weapons[type + 2], transform.position, Quaternion.identity); //객체생성
            GameObject sword2 = Instantiate(weapons[type + 3], transform.position, Quaternion.identity); //객체생성

            sword1.transform.localScale = new Vector3(1.5f, 1.5f, 1f); //검의 크기를 2배로
            sword2.transform.localScale = new Vector3(1.5f, 1.5f, 1f); //검의 크기를 2배로
        ...
        GameObject sword = Instantiate(weapons[type+4], transform.position, Quaternion.identity); //객체생성
    ...
    {
        GameObject floor = Instantiate(weapons[type]); //객체생성
    ...
    public void RestartWeapon() //분신이 만들어졌을 때 애니메이션 동기화
    {
        Transform[] childs0 = null;
        Transform[] childs1 = null;
        Transform[] childs2 = null;
        Transform[] childs3 = null;

        childs0 = weaponLTL.GetComponentsInChildren<Transform>();
        childs1 = weaponLTR.GetComponentsInChildren<Transform>();
        childs2 = floorWeapon1.GetComponentsInChildren<Transform>();
        childs3 = floorWeapon2.GetComponentsInChildren<Transform>();

        if (childs0 != null)
            for (int i = 1; i < childs0.Length; i++)
            {
                if (childs0[i] != transform)
                {
                    childs0[i].gameObject.SetActive(false); //껏다킴으로 동기화
                    childs0[i].gameObject.SetActive(true);
                }
            }
        ...
        if (childs1 != null)
            ...
            {
                if (childs1[i] != transform)
                {
                    childs1[i].gameObject.SetActive(false); //껏다킴으로 동기화
                    childs1[i].gameObject.SetActive(true);
                }
            }
        ...
        if (childs2 != null)
            ...
            {
                if (childs2[i] != transform)
                {
                    childs2[i].gameObject.SetActive(false); //껏다킴으로 동기화
                    childs2[i].gameObject.SetActive(true);
                }
            }

        if (childs3 != null)
            for (int i = 1; i < childs3.Length; i++)
            {
                if (childs3[i] != transform)
                {
                    childs3[i].gameObject.SetActive(false); //껏다킴으로 동기화
                    childs3[i].gameObject.SetActive(true);
                }
            }
